...
public class TaskOrderConsumer {
    ...
    @KafkaListener(topics = "${kafka.order.task.topic}", groupId = "${kafka.order.task.group}")
    public void orderTaskListener(String orderTaskMessage) {
        OrderTask task = null;
        try {
            ...
            log.error("orderTaskListener Error message = {}", orderTaskMessage, e);
        }
    }

    private void sendMessageEvent (OrderSubTask subTask) throws InterruptedException, JsonProcessingException {
//        String inputTask = subTask.getInputTask();
        String inputTask = objectMapper.writeValueAsString(subTask);
    ...
    private void createThread(String latchKey, OrderSubTask subTask) throws InterruptedException {
        countDownLatchManager.addCountDownLatch(latchKey);
