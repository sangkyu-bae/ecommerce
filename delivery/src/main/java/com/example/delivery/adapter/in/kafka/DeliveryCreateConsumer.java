...
//            rollbackOrderEventUseCase.RollbackEventSend(command);
    ...
    @KafkaListener(topics = "${kafka.product.topic}",
            groupId = "${kafka.product.task.group}",
            containerFactory = "batchKafkaListenerContainerFactory")
    public void createDeliveryListenerByBulkInsert(List<String> deliveryJsonList) {
        try {
            List<RegisterDeliveryRequest> requests = new ArrayList<>();

            for (String deliveryJson : deliveryJsonList) {
                RegisterDeliveryRequest request = objectMapper.readValue(deliveryJson, RegisterDeliveryRequest.class);
                requests.add(request);
            }

            List<RegisterDeliveryCommand> commands = requests.stream()
